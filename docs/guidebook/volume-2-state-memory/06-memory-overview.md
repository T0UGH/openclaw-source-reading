---
title: "06｜Memory 总览：OpenClaw 如何让 Agent 真正“有记忆”"
status: draft
chapter: "06"
...
如果只从普通 coding agent 的经验出发，很容易把 memory 理解成两种东西：要么是一段塞进 prompt 的用户画像，要么是一个向量库工具。OpenClaw 的设计更复杂一点：它不是把“记忆”藏在模型里，也不是只给 Agent 一个 search tool，而是把长期事实、每日上下文、检索、主动召回、压缩前保存、后台整理、知识库化这几件事拆成不同层。

本篇先不进入每个子机制的细节，而是给第二卷搭一个总地图：OpenClaw 为什么能说自己“有记忆”。
...
这张图里最重要的是方向：OpenClaw 不是只在回复时“读一段记忆”，而是把记忆文件、插件能力、检索工具、主动召回、压缩前保存和后台晋升连成闭环。
...
这个判断很关键。它意味着 OpenClaw 的 memory 首先是可见、可编辑、可审查的文件系统，而不是某个不可解释的模型内部向量。
...
这和 OpenClaw 的整体架构一致：核心保持 plugin-agnostic，插件通过 manifest、SDK 和 capability contract 接入。Memory 是 OpenClaw 的核心能力，但它仍然以插件化方式进入运行时。

## 第三层：检索工具，把文件变成可用上下文

光有 Markdown 文件还不够。长期运行后，文件会越来越多，用户也不可能每次准确说出关键词。OpenClaw 因此提供两个工具：
...
从 backend 看，OpenClaw 也不是只有一种存储：
...
下一篇会专门讲 Active Memory，这里只需要先明确它在总体系里的位置：
...
这不是用户显式说“记住”的路径，而是系统在长期运行中主动保护上下文连续性的路径。
...
两者一读一写，构成 OpenClaw memory 闭环中最关键的一对。
...
重要的是，Memory Wiki 不替代 active memory plugin。文档明确说：active memory plugin 仍然拥有 recall、promotion、dreaming；`memory-wiki` 是 beside it 的 provenance-rich knowledge layer。

也就是说，Memory Wiki 是长期记忆成熟后的“知识库化”表面，而不是最底层的记忆机制。

## 读源码时应该避免的误解

第一，不要把 memory 简化成 `MEMORY.md`。`MEMORY.md` 很重要，但 OpenClaw 的 memory 包括 daily notes、search、active recall、flush、dreaming、wiki 等多层。
...
OpenClaw 的 memory 不是“模型记住了用户”，而是一个围绕 workspace 文件和 plugin runtime 展开的长期状态系统。
