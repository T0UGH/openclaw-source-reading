...
这里的 blocking memory pass，可以先翻译成“主回复前的阻塞式记忆预检”：在真正生成用户可见回复之前，OpenClaw 先让一个受限的记忆子过程跑一小步，只判断“这轮回复是否需要带上一点长期记忆”。如果有，它把一段很短的记忆摘要塞进主模型的隐藏上下文；如果没有，它返回 `NONE`，主回复照常生成。

本章要回答的不是“OpenClaw 怎么搜索记忆”这一整套问题，而是更窄的一点：**为什么不让主 Agent 自己在需要时调用记忆工具，而要在主回复前额外加一层会增加延迟的阻塞步骤？**
...
Active Memory 的设计就是补这个空档：在符合条件的会话里，OpenClaw 给系统一次短暂、受限、可失败的机会，在主回复之前主动取回相关记忆。它不是让记忆支配回答，而是把“可能相关的用户长期状态”提前放到主模型眼前。

这也符合 OpenClaw 的整体 thesis：OpenClaw 不是只处理当前输入的对话壳，而是一个个人 AI 运行时。Active Memory 负责其中一个很具体的环节：**当用户正在一个持久会话里互动时，让长期记忆有机会在第一句话里自然出现。**
...
这张图有三个重点：Active Memory 挂在 prompt 构建之前；它有多道准入门；它返回的不是用户回复，而是一小段供主模型参考的不可信 metadata。
...
源码里的分工很明确：子 Agent 只允许使用 `memory_search` 和 `memory_get`；找不到强相关内容就返回 `NONE`；找到时只返回紧凑的 plain-text summary。它不是“另一个聊天 Agent”，而是一次受限召回。
...
所以 blocking 不是“为了记忆牺牲响应性”，而是“为了第一句话的个性化，支付一笔有上限的小成本”。如果这笔成本超时、失败、找不到结果，系统就退回普通主回复。
...
Active Memory 的底层检索不是另起炉灶。它复用正常 `memory_search` pipeline。真正的索引、embedding、hybrid search、sync、store 健康度，仍然属于 Memory Search。
