...
flowchart LR
  Msg[消息] --> GW[Gateway]
  Webhook[Webhook] --> GW
  Tick[Heartbeat Tick] --> GW
  Schedule[Cron Schedule] --> GW
  GW --> Run[Agent Run]
```
...
title: 02｜Event Sources：OpenClaw 的 Agent Run 从哪里来 机制图
type: flow
purpose: 用一张正式技术架构图解释“OpenClaw 不是只有用户 prompt，它有哪些事件来源？”
prompt_seed: 生成一张 16:9 中文技术架构图，主题是 OpenClaw 源码阅读第 02 篇：Event Sources：OpenClaw 的 Agent Run 从哪里来。图中只保留少量标签，突出层次、边界和主链路；高对比、无 logo、无水印，不要装饰性插画。
