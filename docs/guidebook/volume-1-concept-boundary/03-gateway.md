...
title: 03｜Gateway：为什么 OpenClaw 的入口不是 CLI 机制图
type: flow
purpose: 用一张正式技术架构图解释“为什么 Gateway 是 OpenClaw 的第一运行边界？”
prompt_seed: 生成一张 16:9 中文技术架构图，主题是 OpenClaw 源码阅读第 03 篇：Gateway：为什么 OpenClaw 的入口不是 CLI。图中只保留少量标签，突出层次、边界和主链路；高对比、无 logo、无水印，不要装饰性插画。
