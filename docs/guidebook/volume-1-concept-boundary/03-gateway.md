...
前一篇已经把 Event Sources 摊开了：消息、heartbeat、cron、webhook、节点和控制面请求都可能触发运行。于是下一个问题变成：**这些来源为什么要先经过同一个 Gateway？**

OpenClaw 的第一边界是 **Gateway**：一个长期运行的入口层，负责接住真实世界里的消息、控制端连接、设备节点、heartbeat、cron 等事件，再把它们转成内部 agent run。

这不是“CLI 换成 WebSocket”这么简单。Gateway 先处理协议、身份、事件归属和连接状态，后面才谈 session、workspace、memory、automation 和 delivery。
...
这张图的重点是“先过边界，再进 agent loop”：消息、控制端、节点和系统事件都先进入 Gateway，后面才谈 session、上下文组装和模型执行。
...
| 容易误读 | 更准确理解 |
|---|---|
| Gateway = 聊天 adapter | Gateway = 第一运行边界 |
| Gateway = API server | Gateway = 协议、身份、事件和连接入口 |
| Gateway = Agent Loop | Gateway 负责把事件归属清楚，再送入 Agent Loop |
| Gateway = CLI 替代 | CLI / UI 只是 Gateway 的 control-plane client 之一 |
...
- 能解释 Gateway 为什么是第一运行边界，而不只是聊天 adapter 或 API server。
- 能区分 Gateway、Agent Loop、CLI / UI control-plane client 各自承担什么。
- 能理解身份、连接、协议、幂等和事件归属为什么必须在入口层处理。
