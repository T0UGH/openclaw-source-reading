...
title: 00｜给 Claude Code 用户的 OpenClaw 概念迁移表 机制图
type: architecture
purpose: 用一张正式技术架构图解释“哪些 Claude Code 心智模型不能直接套到 OpenClaw？”
prompt_seed: 生成一张 16:9 中文技术架构图，主题是 OpenClaw 源码阅读第 00 篇：给 Claude Code 用户的 OpenClaw 概念迁移表。图中只保留少量标签，突出层次、边界和主链路；高对比、无 logo、无水印，不要装饰性插画。
