...
title: 13｜Reply Shaping：为什么聊天渠道需要一层回复整形 机制图
type: flow
purpose: 用一张正式技术架构图解释“为什么模型输出不能直接等于聊天消息？”
prompt_seed: 生成一张 16:9 中文技术架构图，主题是 OpenClaw 源码阅读第 13 篇：Reply Shaping：为什么聊天渠道需要一层回复整形。图中只保留少量标签，突出层次、边界和主链路；高对比、无 logo、无水印，不要装饰性插画。
