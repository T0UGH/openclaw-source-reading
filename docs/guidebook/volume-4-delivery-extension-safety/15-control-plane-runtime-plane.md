...
图片里的竖向分层很关键：Control Plane 不是 Runtime Plane 的“前置目录页”，而是一个可频繁读取、低副作用、可诊断的治理表面；Runtime Plane 才是插件真正执行 provider / channel / tool / hook / service 的地方。下面几节都围绕这条分界解释 manifest-first 和 lazy loading。
...
这解决的是 Gateway 类 runtime 的基本问题：系统如果要回答“现在有哪些插件可用”“这个插件缺什么配置”“某个 provider 的 key 从哪里来”“某个 channel 是哪个插件拥有的”，不能每次都先把插件 runtime 启动一遍。否则，配置检查本身就会变成副作用入口，Control Plane 也会退化成“顺手把运行时跑起来”。
...
也就是说，OpenClaw 的插件加载不是一个布尔值，而是一套分层协议。关键不是模式名字有多少，而是每一步都在回答“现在是否真的需要进入运行面”：
...
这条边界把插件系统从“工具目录”推进到个人 AI runtime 的能力治理层。它的价值不是少 import 几个文件，而是让“可发现、可配置、可诊断”和“可执行、可连外部服务、可产生副作用”分属不同平面。下一章讲安全时，会继续沿着这条线往下看：当真实消息、设备、节点、文件、记忆、定时任务和投递都进入同一个 runtime，安全就不再只是“命令能不能执行”。

## 本章检查点

读完这一章，你应该能：

- 能解释 Control Plane 和 Runtime Plane 为什么要分开。
- 能说明 manifest-first、public surface、lazy runtime loading 各自降低什么风险。
- 能判断插件为什么不能在系统启动时一上来全加载。
