...
如果你从普通 coding agent 的经验出发，很容易把“插件”理解成一组工具：启动时把所有插件加载进来，把工具表拼好，模型需要什么就调用什么。

OpenClaw 不能这么做。它不是只在一个终端里等待用户输入的工具箱，而是一个长期运行的个人 AI runtime：Gateway 要先启动，真实消息渠道要接入，控制端要查询状态，配置页要展示插件能力，定时任务和后台会话还会在不同时间醒来。这里最关键的分界线就是：**Control Plane 先知道“有哪些能力、如何配置、什么时候该加载”；Runtime Plane 再真正执行这些能力。**
...
OpenClaw 的插件不是“启动时全量 import 的工具目录”，而是外部世界能力的所有权边界。manifest-first 让 Gateway 可以在不执行插件代码的前提下完成发现、校验、诊断、配置 UI、setup/onboarding 和 activation planning；runtime lazy loading 则保证只有当某个 provider、channel、route、command 或 capability 真的被需要时，才进入插件的真实运行时。

这不是单纯的性能优化，而是个人 AI runtime 的启动边界和安全边界：控制面要足够轻、可解释、可诊断；运行面要足够晚、足够窄、只为当前事件加载必要能力。
...
读这张图时要注意：上半部分回答“系统知道什么”，下半部分回答“系统执行什么”。OpenClaw 把这两个问题拆开，才不会在 Gateway 刚启动时就把所有渠道、模型、媒体、搜索、语音、hook 和服务全部拉进进程。
...
这解决的是 Gateway 类 runtime 的一个基本问题：如果系统要回答“现在有哪些插件可用”“这个插件缺什么配置”“某个 provider 的 key 从哪里来”“某个 channel 是哪个插件拥有的”，它不能每次都先把插件 runtime 启动一遍。否则配置检查本身就会变成副作用入口。
...
`docs/plugins/architecture.md` 里把插件系统拆成四层：Manifest + discovery、Enablement + validation、Runtime loading、Surface consumption。真正体现 Control Plane / Runtime Plane 分界的是 activation planning。

manifest 里的 `activation` 字段可以声明：哪些 provider、command、channel、route、capability 会触发这个插件进入加载计划。文档同时强调，它不是生命周期 hook，也不替代 `register(...)`。它只是 planner metadata：帮助系统在真正运行前缩小候选插件范围。
...
这些事件不应该都触发同一套“全插件加载”。一个 provider 查询只需要相关 provider 插件；一个 channel setup 只需要相关 channel 插件；一个 CLI command 只需要声明该 command 的插件。activation planning 的价值就是：在 Control Plane 上先回答“这次最多需要谁”。

## 3. loader.ts：加载意图被拆成多种模式

`src/plugins/loader.ts` 不是简单的 `import all plugins`。从 `PluginLoadOptions` 就能看出，加载过程带着明确意图：
...
后面的 `resolvePluginRegistrationPlan(...)` 把这些意图转成内部执行计划：`setup-only`、`setup-runtime`、`discovery`、`full` 等不同模式会决定是否加载 setup entry、是否跑 runtime capability policy、是否执行 full activation-only registrations。

也就是说，OpenClaw 的插件加载不是一个布尔值，而是一套分层加载协议：
...
`src/plugins/public-surface-loader.ts` 处理 bundled plugin 的 public artifact 加载。它做了几件值得注意的事：解析 public surface 位置、缓存模块、通过 `openBoundaryFileSync` 检查路径边界、用文件身份校验防止验证后文件被替换，再通过 jiti 或 require 加载。

这说明 OpenClaw 对“能被控制面读取的公共表面”也不是随便 import。即使只是加载 bundled plugin 的 public artifact，也要有：
...
这不是写框架时的洁癖，而是长期运行进程必须面对的现实：一旦插件系统允许外部扩展，任何“读取元数据”都可能变成边界问题。
...
第五，**安全边界更清楚**。manifest metadata 可以被控制面读取；runtime 行为必须等到明确触发、明确范围、明确注册模式之后才进入进程。这让下一章要讲的安全与隔离有了落点。

## 6. 和“工具列表”的区别

普通工具列表的思路是：工具越早注册越好，模型看到完整菜单之后自己选。

OpenClaw 的思路是反过来的：真实世界事件先进入 Gateway；Gateway 根据 channel、session、agent、provider、route、command 和 capability 判断这次需要哪些插件表面；插件 runtime 再按需加载，把能力注册到 registry；最后由 agent run 或 delivery 层消费这些能力。

这也是为什么插件系统不能被写成“工具扩展机制”。工具只是插件能力的一种。插件还可能拥有：
...
本章的保留结论是：**manifest-first 负责让 OpenClaw 在不启动插件 runtime 的情况下理解世界；lazy runtime loading 负责让真正的能力只在相关事件触发时进入运行面。**

这条边界把插件系统从“工具目录”提升成个人 AI runtime 的能力治理层。下一章讲安全时，会继续沿着这条线往下看：当真实消息、设备、节点、文件、记忆、定时任务和投递都进入同一个 runtime，安全就不再只是“命令能不能执行”。
