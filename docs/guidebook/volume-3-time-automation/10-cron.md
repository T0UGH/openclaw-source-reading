...
title: 10｜Cron：精确调度、隔离执行与结果投递 机制图
type: lifecycle
purpose: 用一张正式技术架构图解释“Cron 和 Heartbeat 的边界是什么？”
prompt_seed: 生成一张 16:9 中文技术架构图，主题是 OpenClaw 源码阅读第 10 篇：Cron：精确调度、隔离执行与结果投递。图中只保留少量标签，突出层次、边界和主链路；高对比、无 logo、无水印，不要装饰性插画。
