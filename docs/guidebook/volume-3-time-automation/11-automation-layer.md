...
前两篇分别讲了 Heartbeat 和 Cron。到这里需要收一下：OpenClaw 不只有一种“自动化”。它把不同类型的自动运行拆成多种机制：Heartbeat、Cron、Background Tasks、Task Flow、Hooks、Standing Orders。

如果把这些东西混在一起，读者很容易以为 OpenClaw 只是“能定时跑 Agent”。它的设计价值在于：没有用一个巨型 scheduler 覆盖所有问题，而是让每种自动化机制负责自己的边界。
...
再往上还有 Task Flow：它处理的不是单个后台任务，而是多步骤流程、revision 和 sync mode 的编排。

所以，OpenClaw 的自动化不是一个单独功能，而是一组边界清楚的运行时表面。
...
这张图想表达的是：OpenClaw 自动化层不会把所有事情都交给 cron，也不会把 Tasks 当成另一个 scheduler，而会按时间、工作状态、事件、规则、流程五个维度拆开。配图之后再逐层解释，读者会更容易把“触发”“执行”“记录”三件事分开。
...
Heartbeat 负责“近似周期地醒来看看”。它默认每 30 分钟，可以读取 `HEARTBEAT.md` 或 prompt，在主会话中检查是否有需要用户注意的东西。

它关注的不是精确时间，重点在低频 presence：
...
所以 Heartbeat 是一种“有事提醒、没事消失”的自动化，而不是把一组精确 job 排进时间表。
...
和 Heartbeat 最大的区别是：Cron 代表一个明确 job。所有 cron executions 都创建 background task records，并可以通过 announce、webhook 或 none 控制结果投递。

如果用户说“明早 9 点提醒我”或“每天 7 点发简报”，这不是 heartbeat checklist，而应该落成 Cron job。

## 第三类：Background Tasks，追踪 detached work

Background Tasks 不是 scheduler。`docs/automation/index.md` 写得很清楚：The background task ledger tracks all detached work。它记录 ACP runs、subagent spawns、isolated cron executions、CLI operations 等离线工作，但不决定这些工作何时被触发。
...
Standing Orders 是另一种容易被忽视的自动化。它不会立即触发某个 run，而是给 Agent 一组长期操作规则和授权边界，通常放在 workspace 文件里，比如 `AGENTS.md`。

它回答的是：Agent 在未来每次运行时，应该持续遵守哪些规则？

这和 Cron 可以组合：Cron 负责“什么时候做”，Standing Orders 负责“以什么原则做”。它也不同于 Hooks：Hooks 是事件脚本，Standing Orders 是进入 Agent prompt 的长期行为约束。
...
| 需求 | 应该用什么 | 原因 |
| --- | --- | --- |
| 每 30 分钟左右看看有没有重要消息 | Heartbeat | 近似周期、聚合检查、没事静默 |
...
这也是 OpenClaw 不只是“一个有工具的聊天 Agent”的原因。它在把真实世界中的时间、事件、后台工作和长期规则接入 agent runtime。
...
OpenClaw 的自动化层不是单个 scheduler，而是一组运行时边界：时间触发、周期检查、后台账本、流程编排、事件反应、长期规则各管一段。
