...
对普通 coding agent 来说，系统一般只有一种启动方式：用户发一条 prompt，Agent 读上下文、调用工具、返回结果。没有用户输入，就没有 agent run。

OpenClaw 不一样。它要做的是长期在线的个人 AI 运行时：它不只响应用户，也要在合适的时间醒来，检查有没有需要提醒、跟进、汇报的事情。但它又不能每次醒来都骚扰用户。所以 Heartbeat 要解决的不是“定时跑任务”这么简单，而是：**如何让 Agent 拥有低频存在感，同时保持安静。**

## 本篇结论

Heartbeat 是 OpenClaw 的**周期性主会话 turn**。它让 Agent 在没有用户输入时也能进入一次主 session，读取 `HEARTBEAT.md` 或配置 prompt，检查待关注事项；如果没有任何需要提醒的内容，就回复 `HEARTBEAT_OK`，由系统过滤掉，不打扰用户。
...
所以 Heartbeat 的产品意义是：**让个人 AI 在真实时间里有一种克制的醒来机制：没事就沉默，有事才出现。**
...
这张图的核心不是“定时器触发模型”，而是从 tick 到静默/投递之间有一整套边界：启用状态、active hours、session、上下文模式、ack 过滤和 delivery target。
...
最容易误解的是把 Heartbeat 当成“简化版 Cron”。这会把两者都讲错。

`docs/gateway/heartbeat.md` 直接说：Heartbeat runs periodic agent turns in the main session。它是一个周期性主会话 turn。它的默认 prompt 也不是“执行某个任务”，而是：读取 `HEARTBEAT.md`，遵守其中清单；不要从旧聊天里推断或重复任务；如果没事，回复 `HEARTBEAT_OK`。

这说明 Heartbeat 的目标是“检查状态”，不是“精确完成一个 job”。它适合：

- 定期看看 inbox / calendar / reminders 是否有需要提醒的东西；
- 检查长期关注事项是否有变化；
- 以低频方式向用户 surfacing 重要信息；
- 批量处理多个轻量检查项，避免每个检查都单独建任务。

Cron 则是精确时间轴：`at`、`every`、cron expression、timezone、delivery、isolated execution。下一篇会展开。

## `HEARTBEAT.md`：周期醒来时读什么

Heartbeat 可以直接用配置里的 prompt，也可以依赖 workspace 里的 `HEARTBEAT.md`。这和前面讲的 Workspace Files 一脉相承：OpenClaw 把长期运行规则拆成不同文件，`HEARTBEAT.md` 专门承载“周期醒来时要看什么”。

这避免了一个常见问题：如果把周期检查事项混在普通 `AGENTS.md` 或会话历史里，Agent 每轮普通回复也会看到 heartbeat-only instructions，容易污染正常对话。OpenClaw 的文档明确说，当 heartbeat 用 `0m` 禁用时，正常 runs 也会从 bootstrap context 中省略 `HEARTBEAT.md`。

也就是说，`HEARTBEAT.md` 是一种场景化记忆：只在系统醒来检查时有意义，不应该永久压在每次用户对话里。

## `HEARTBEAT_OK`：让“没事”真的不打扰

Heartbeat 最关键的产品细节是 `HEARTBEAT_OK`。
...
`src/auto-reply/heartbeat-filter.ts` 还能看到另一个上下文卫生细节：`filterHeartbeatPairs` 会识别 heartbeat user message 和 assistant 的 OK response，把这对消息从历史里过滤掉。否则每 30 分钟一次的“检查/没事”会很快污染主会话上下文。

这就是 Heartbeat 的克制：系统确实醒来了，但没有事情时，它不会留下可见噪音，也尽量不把空检查塞进后续上下文。

## Active hours：不是 24 小时都该提醒

Heartbeat 还有 active hours。文档里的例子是可以设置 `activeHours: { start, end, timezone }`，窗口之外的 heartbeat 会跳过，直到下一个窗口内 tick。

这看起来只是一个配置项，但对个人 AI runtime 很重要。因为 Heartbeat 不是任务调度器，它带有“陪伴 / 关注 / 提醒”的意味。半夜把用户叫醒和白天提醒用户检查事项，产品感完全不同。

所以 OpenClaw 把时间也变成运行边界：同样的周期检查，在不同时间段可能应该沉默。

## lightContext 与 isolatedSession：控制成本和历史污染

Heartbeat 默认使用主会话上下文，这让它适合做 context-aware 的周期检查。但如果每半小时都带完整历史，成本会很高，也可能让旧任务不断被误读。
...
这两个选项说明 OpenClaw 并不把 Heartbeat 设计成“无限复读主会话”。它可以根据任务类型选择上下文重量：日常陪伴可以用主 session，纯检查清单可以用 light context，成本敏感或污染风险高时可以用 isolated session。

## Delivery：醒来之后要不要送到真实渠道

Heartbeat 的 target 默认是 `none`，也就是可以运行但不外部投递。也可以设为 `last`，送到最近联系的渠道；或者显式指定 channel、recipient、accountId。

这同样体现了 OpenClaw 和 CLI agent 的差异。CLI agent 只要打印输出即可；OpenClaw 的 heartbeat 结果要进入真实世界渠道，因此必须知道：送到哪里、是否允许 DM、是否包含 reasoning、是否启用 typing indicator、是否有多账号 channel。

所以 Heartbeat 不只是调度问题，它还连着 delivery runtime。
...
Heartbeat 让 OpenClaw 从“只等用户输入”变成“会定期醒来看看”。但它真正成熟的地方，不在于会醒来，而在于会克制：有事提醒，没事静默；需要上下文时用主 session，成本敏感时用 light/isolated context；该投递时进入真实渠道，不该打扰时保持安静。
