...
title: 09｜Heartbeat：为什么 OpenClaw 会在没有用户输入时醒来 机制图
type: lifecycle
purpose: 用一张正式技术架构图解释“Heartbeat 解决什么问题？”
prompt_seed: 生成一张 16:9 中文技术架构图，主题是 OpenClaw 源码阅读第 09 篇：Heartbeat：为什么 OpenClaw 会在没有用户输入时醒来。图中只保留少量标签，突出层次、边界和主链路；高对比、无 logo、无水印，不要装饰性插画。
