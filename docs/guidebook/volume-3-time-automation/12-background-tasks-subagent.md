...
在普通聊天式 Agent 里，用户通常只关心当前这一轮：我发一句话，模型做一点工具调用，然后给一个最终回答。即使它中间跑了脚本或调用工具，大多数时候也都被理解成“这轮对话的一部分”。
...
所以 Tasks 的产品意义不是“多一个列表命令”，而是让 OpenClaw 的后台自动化变得可见、可取消、可审计。
...
这句话很关键。Tasks 不决定工作什么时候运行。真正决定时间的，是 Cron 和 Heartbeat；决定事件反应的，是 Hooks；决定长期规则的，是 Standing Orders。Tasks 只负责记录：某个 detached work 被创建了、开始了、结束了、失败了还是丢了。

这也是为什么文档说：Tasks are records, not schedulers。

如果把 Tasks 误解为调度器，就会把自动化层讲乱。正确理解是：Tasks 是运行时账本。
...
这让读者能把边界记住：**离开主对话同步路径、需要后台追踪的工作，才进入 task ledger。**
...
`lost` 不是“失败”的同义词，而是 runtime ownership 消失：
...
- CLI task：chat-backed CLI task 看 live run context，而不是只看 chat session row。

这说明 OpenClaw 没有把 task record 当成孤立数据库行。它会和背后的真实 runtime 进行 reconciliation。记录说它还在跑，但 runtime 不再拥有它，就需要进入 `lost`，并在 audit 中暴露。
...
这和前面的 Heartbeat、Cron 连起来了：后台工作完成不是等用户回来问，而是由运行时主动把结果送回用户所在的渠道或 session。
...
## Subagent：不是“另一个聊天窗口”，而是带 requester 的 detached work
...
这说明 subagent 的难点不只是“并发执行”，而是“完成之后回到哪里”。OpenClaw 必须保留发起者、渠道、线程、账号这些 routing 信息，否则后台结果会丢到错误地方。
...
Background Tasks 是 OpenClaw 自动化可信度的底座。Cron、subagent、ACP、CLI 都可以把工作丢到后台，但只有 task ledger 能让这些工作变得可见、可取消、可审计，并在完成时回到正确的用户和渠道。
